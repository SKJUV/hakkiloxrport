...
export const useAppStore = (): Omit<AppContextType, keyof ReturnType<typeof useDummyHandlers>> => {
  ...
  const [portfolioScroll, setPortfolioScroll] = useState<number>(0);
  const portfolioScrollRef = useRef(portfolioScroll);
  useEffect(() => { portfolioScrollRef.current = portfolioScroll; }, [portfolioScroll]);
  ...
  useEffect(() => {
    ...
    const gameLoop = (timestamp: number) => {
        ...
        // --- Physics & Camera ---
        const devMode = devModeRef.current;
        ...
        if (!devMode) {
            ...
            proposedPos[1] = cameraRef.current.position[1];
            proposedPos[2] = cameraRef.current.position[2];

            // Calculate pseudo velocity based on scrolling delta to modulate sound!
            const scrollVelocity = (progress - prevScrollProgressRef.current) / dt;
            prevScrollProgressRef.current = progress;
            const scrollSpd = Math.abs(scrollVelocity) * 15.0; // Scale velocity

            smoothScrollVelocityRef.current += (scrollSpd - smoothScrollVelocityRef.current) * 0.1;
            cameraVelocityRef.current[0] = Math.sin(cameraRef.current.rotation[1]) * smoothScrollVelocityRef.current;
            cameraVelocityRef.current[1] = 0;
            cameraVelocityRef.current[2] = Math.cos(cameraRef.current.rotation[1]) * smoothScrollVelocityRef.current;

        } else {
            // --- Manual Flight Mode (Original Gameplay) ---
            ...
            pitchInput = (keys.has('arrowdown') ? 1 : 0) - (keys.has('arrowup') ? 1 : 0);
            if (controls.invertPitch) pitchInput = -pitchInput;
            yawInput = (keys.has('arrowright') ? 1 : 0) - (keys.has('arrowleft') ? 1 : 0);
  ...
  return {
    ...
    devMode, setDevMode, portfolioScroll, setPortfolioScroll
  };
};
...
const useDummyHandlers = () => ({
  ...
  devMode: false, setDevMode: () => {}, portfolioScroll: 0, setPortfolioScroll: () => {}
});
